...
def interest_total():
    if request.method == 'POST':
        ...
        if not valid_time(time):
            ...
            message = 'If you invest ${} at {}% for {} year(s) you will get ${} worth of interest'.format(
                initial, rate, time, total)
        return render_template('interest_form.html', calc_total=total, message=message)
    return render_template('interest_form.html', calc_total=None)
